...
#1. Вывести список всех файлов в папке html
# Переходим в папку html
cd /var/www/html

# Выводим весь список файлов (в данном случае используется флаг -a, который отвечает за отображение скрытых файлов и папок)
ls -a
...
# С помощью поисковика grep находим все упоминания строки "404" в ./access.log
grep "404" ./access.log


#3. На основании файла 404.html сделайте файл 403.html заменив в его содержимом все упоминания 404 на 403
# Переходим в папку html с папки logs
cd ../www/html
...
#4.1 Текущий пользователь должен иметь права на чтение/запись
# Устанавливаем текущему пользователю права на чтение/запись
chmod u+rw ./index.html 

#4.2 Группа www-data на чтение
# Добавляем группу www-data файлу ./index.html
chgrp www-data ./index.html

# Устанавливаем группе права на чтение
chmod g+r ./index.html 

#4.3 Все остальные не имеют доступа к файлу
